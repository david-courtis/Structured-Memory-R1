...
class MemoryBank:
    ...
    def schema_summary(self) -> dict:
        """Public compact schema summary for retrieval planning."""
        return self._schema_summary()

    def execute_retrieval_plan(self, plan: dict, topk: int = 30) -> List[dict]:
        """
        Execute a retriever-agent search plan over the fixed memory tree.

        Plan schema:
        {
          "levels": [{"level": 0, "keys": ["Alice"]}, ...],
          "selected_ids": ["0", "4"],
          "stop": true
        }
        """
        ...
        frontier: List[StructuredMemoryNode] = [self._root]
        levels = plan.get("levels", [])
        if isinstance(levels, list):
            for step in levels:
                if not isinstance(step, dict):
                    continue
                keys = [str(key).strip() for key in step.get("keys", []) if str(key).strip()]
                if not keys:
                    continue
                next_frontier: List[StructuredMemoryNode] = []
        ...
        candidate_ids: List[str] = []
        seen_ids: Set[str] = set()
        selected_ids = [str(entry_id).strip() for entry_id in plan.get("selected_ids", []) if str(entry_id).strip()]
        for entry_id in selected_ids:
            if entry_id in self._entries and entry_id not in seen_ids:
                candidate_ids.append(entry_id)
                seen_ids.add(entry_id)
        ...
        for rank, entry_id in enumerate(candidate_ids):
            entry = self.get(entry_id)
            if entry is None:
                continue
        ...
        return [
            self._make_contextual_entry(entry, score)
            for score, entry in scored_entries[:topk]
        ]
