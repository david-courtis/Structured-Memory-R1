...
class TestComputeMemoryManagerReward:
    ...
    def test_with_tree_retrieval_answer_function(self):
        def mock_answer_fn(question, memories):
            assert len(memories) == 1
            assert "Buddy" in memories[0].get("text", "")
            return "Buddy"

        output = (
            '{"memory": ['
            '{"id": "0", "text": "Alice adopted a dog named Buddy", '
            '"event": "ADD", "speaker": "Alice", "topic": "dog", "path": ["Alice", "dog"]},'
            '{"id": "1", "text": "Bob bought a guitar", '
            '"event": "ADD", "speaker": "Bob", "topic": "music", "path": ["Bob", "music"]}'
            ']}'
        )
        score = compute_memory_manager_reward(
            old_memory=[],
