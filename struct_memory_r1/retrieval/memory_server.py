...
class MemoryStore:
    ...
    def __init__(self):
        ...
        self.structured_bank = MemoryBank()

    def load_memories(self, memories: List[Dict]):
        """Load memories and build TF-IDF index."""
        self.memories = memories
        self.structured_bank = MemoryBank.from_list(memories)
        if not memories:
            return
    ...
    def retrieve(
        ...
        structured_hits = self.structured_bank.retrieve(query, topk=topk, speaker=speaker)
        if structured_hits:
            return [
                {
