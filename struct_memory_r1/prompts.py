...
def make_memory_manager_training_prompt(
    ...
    if old_memory:
        ...
        for item in old_memory:
            path = item.get("path") or [item.get("speaker", "global"), item.get("topic", "general")]
            cursor = structured_tree
            for part in path:
                cursor = cursor.setdefault(str(part), {})
            cursor.setdefault("_facts", []).append(item.get("text", ""))
...
Structured Memory Tree:
{json.dumps(structured_tree, indent=2)}
